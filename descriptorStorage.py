...
def init_db(filename: str, path_to_images: str, matcher: FeatureMatcher, mapping_file: str = 'data.txt',
            des_dataset: str = 'DatasetDescriptors', kp_dataset: str = 'DatasetKeypoints'):
    ...
    with h5py.File(filename, 'w') as f:
        ...
        for image in imgs:
            ...
            if (not image in mapping) or not mapping[image]:
                ...
                for point in kp:
                    temp = (
                        point.pt[0], point.pt[1], point.size, point.angle, point.response, point.octave, point.class_id)
...
def update_db(filename: str, path_to_images: str, matcher: FeatureMatcher, mapping_file: str = 'data.txt',
              des_dataset: str = 'DatasetDescriptors', kp_dataset: str = 'DatasetKeypoints'):
    ...
    with h5py.File(filename, 'a') as f:
        if not des_dataset in f.keys():
            # TODO: estimate number of kp beforehand
            des_set = f.create_dataset(des_dataset, (500000, 128), maxshape=(None, 128))
        des_set = f[des_dataset]
        if not kp_dataset in f.keys():
            kp_set = f.create_dataset(kp_dataset, (500000, 7), maxshape=(None, 7))
        ...
        # TODO: make mapping as a h5 datasets (filenames, intervals)
        with open(mapping_file) as jfile:
            mapping = json.load(jfile)
        ...
        for image in imgs:
            #des_set.create_group(image, (100, 128), maxshape=(None, 128))
            img = cv.imread(image, cv.IMREAD_GRAYSCALE)
            if (not image in mapping) or not mapping[image]:
                # TODO: create dataset after knowing des and kp dimensionality
                kp, des = matcher.detect(img)
                des_count, _ = des.shape
                keypoints = []
                for point in kp:
                    ...
                    keypoints.append(temp)
                end_index = start_index + des_count
                # TODO: chcek if there is space in db, otherwise resize
                des_set[start_index:end_index, ...] = des
                kp_set[start_index:end_index, ...] = keypoints
...
def get_des(filename: 'str', image_path: 'str', mapping_file: str = 'data.txt',
            des_dataset: str = 'DatasetDescriptors'):
...
def get_kp(filename: 'str', image_path: 'str', mapping_file: str = 'data.txt', kp_dataset: str = 'DatasetKeypoints'):
    ...
    with h5py.File(filename, 'r') as f:
        ...
        for i in range(mapping[image_path][0], mapping[image_path][1]):
            temp = cv.KeyPoint(x=kpset[i][0], y=kpset[i][1], _size=kpset[i][2], _angle=kpset[i][3],
                               _response=kpset[i][4], _octave=kpset[i][5], _class_id=kpset[i][6])
            kp.append(temp)
